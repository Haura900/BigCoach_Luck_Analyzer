(function () {
  ...
  const elements = {
    ...
    historyRefreshNote: document.querySelector("#history-refresh-note"),
  ...
  function render() {
    ...
    renderTrend();
    renderHistory();
    elements.historyPlayer.dispatchEvent(new Event("input"));
  }
  ...
  function buildBookmarklet() {
    ...
    const updateHistoryBookmarklet = () => {
      ...
      const refreshTaskIds = records
        .filter((record) => record.taskId && record.calculationVersion !== analyzer.CALCULATION_VERSION)
        .map((record) => String(record.taskId));
      const refreshBatchId = refreshTaskIds.length ? hashText(refreshTaskIds.slice().sort().join("|")) : "";
      elements.historyRefreshNote.hidden = refreshTaskIds.length === 0;
      elements.historyRefreshNote.textContent = refreshTaskIds.length
        ? `旧計算の${refreshTaskIds.length}対局も再取得します。このボタンをブックマークへ登録し直してください。`
        : "";
      if (!target) {
